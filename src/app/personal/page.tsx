...
type TabKey = "overview" | "coverage" | "pipeline" | "restraint";
const VALID_TABS: TabKey[] = ["overview", "coverage", "pipeline", "restraint"];
...
export default async function PersonalPage({ searchParams = {} }: Props) {
  ...
  const activeTab: TabKey = (VALID_TABS as string[]).includes(tabParam || "")
    ? (tabParam as TabKey)
    : "overview";
  ...
  return (
    <>
      ...
      <main className="cmk-personal cmk-personal--full">
        ...
        <nav className="proposal-tabs cmk-personal-tabs" aria-label="Personal sections">
          <Link
            href="/personal"
            data-tab="overview"
            className={`proposal-tab${activeTab === "overview" ? " proposal-tab-active" : ""}`}
          >
            Overview
          </Link>
          <Link
            href="/personal?tab=coverage"
            data-tab="coverage"
            className={`proposal-tab${activeTab === "coverage" ? " proposal-tab-active" : ""}`}
          >
            Coverage
          </Link>
          <Link
            href="/personal?tab=pipeline"
            data-tab="pipeline"
        ...
        {/* ─── OVERVIEW ─────────────────────────────────────────────── */}
        {activeTab === "overview" && (
          <div className="cmk-personal-tab cmk-personal-tab--overview">
        ...
        {/* ─── COVERAGE ─────────────────────────────────────────────── */}
        {activeTab === "coverage" && (
          <div className="cmk-personal-tab cmk-personal-tab--coverage">
              ...
              <div className="cmk-personal-fillrates">
                {data.slot_fill_rates.map((s) => (
                  <div key={s.slot_id} className="cmk-personal-fill-row">
                    ...
                    <span
                      className={`cmk-personal-fill-cat cmk-personal-fill-cat-${s.category}`}
                    >
                      {s.category}
                    </span>
            ...
            <section className="cmk-stack-panel cmk-stack-panel--lemon">
              <div className="cmk-stack-panel-head">
                ...
                <div className="cmk-stack-panel-meta">most common gaps across the book</div>
              </div>
              {data.top_quests.length === 0 ? (
                <p className="cmk-discovery-empty">
                  No active gaps — every slot is filled across the board.
                </p>
              ) : (
                  ...
                  {data.top_quests.map((q) => (
                    <li key={q.slot_id} className="cmk-personal-quest-row">
                      ...
                      <span className="cmk-personal-quest-lbl">
                        lead{q.unknown_count === 1 ? "" : "s"} need{" "}
                        <strong>{q.label.toLowerCase()}</strong>
                      </span>
                    </li>
                  ))}
                </ol>
        ...
        {/* ─── PIPELINE ─────────────────────────────────────────────── */}
        {activeTab === "pipeline" && (
          <div className="cmk-personal-tab cmk-personal-tab--pipeline">
            ...
            <section className="cmk-stack-panel cmk-stack-panel--peach">
              <div className="cmk-stack-panel-head">
                <h2 className="cmk-stack-panel-title">🟢 SCORE tag distribution</h2>
                <div className="cmk-stack-panel-meta">your hot-tag spread across the pipeline</div>
              </div>
              {data.hot_tag_counts.length === 0 ? (
                <p className="cmk-discovery-empty">
                  No 🟢 SCORE tags set across active leads.
                </p>
              ) : (
                <ul className="cmk-personal-hottags">
                  {data.hot_tag_counts.slice(0, 16).map((t) => (
        ...
        {/* ─── RESTRAINT ────────────────────────────────────────────── */}
        {activeTab === "restraint" && (
          <div className="cmk-personal-tab cmk-personal-tab--restraint">
            <section className="cmk-stack-panel cmk-stack-panel--lavender">
              <div className="cmk-stack-panel-head">
              ...
              <div className="cmk-personal-restraint-headline">
                <div className="cmk-personal-restraint-num">
                  {data.restraint_30d == null ? "—" : `${data.restraint_30d}%`}
                </div>
                <div className="cmk-personal-restraint-caption">
                  {data.restraint_30d == null
                    ? "No heartbeat activity yet in the 30-day window."
